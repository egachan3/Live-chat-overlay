...
(function (): void {
  ...
  /** 積み上げ型の表示位置の型・デフォルト値（overlay.tsと同じ値を使う） */
  ...
  function init(): void {
    ...
    const displayModeFlow = document.getElementById(
      "displayModeFlow"
    ) as HTMLInputElement | null;
    const stackPositionSection = document.getElementById("stackPositionSection");
    const stackPositionRight = document.getElementById(
      "stackPositionRight"
    ) as HTMLInputElement | null;
    const stackPositionLeft = document.getElementById(
      "stackPositionLeft"
    ) as HTMLInputElement | null;
    ...
    const updateStackPositionAvailability = (): void => {
      stackPositionSection.classList.toggle(
        "stack-position-disabled",
        !displayModeStack.checked
      );
    };
    ...
    const handleDisplayModeChange = (): void => {
      const displayMode: DisplayMode = displayModeFlow.checked ? "flow" : "stack";
    ...
    const handleStackPositionChange = (): void => {
      const stackPosition: StackPosition = stackPositionLeft.checked ? "left" : "right";
